import 'package:flutter/material.dart';

class ColorSelect {
  static Color importantColor = const Color(0xFFF85977);
  static Color primaryColor = const Color(0xFF5946D2);
  static Color primaryLightColor = const Color(0xFFC8BFFF);
  static Color lightPurpleBackground = const Color(0xFFB0A2F2);
  static Color darkPurple = const Color(0xFF988CD0);
  static Color grayColor = const Color(0xFF777679);
  static Color lightGrayColor = const Color(0xFFe3e3e4);
  static Color darkGrayColor = const Color(0xFF201F24);
  static Color darkSecondaryColor = const Color(0xFF544794);
  static Color darkSecondaryContainerColor = const Color(0xFF6B60A1);
  static Color darkSecondaryGrayColor = const Color(0xFF979397);
  static Color outlinedPurple = const Color(0xFF9373FF);
}